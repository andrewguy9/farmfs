...
class Path:
    ...
    def is_contained_in(self, root: "Path") -> bool:
        """Return True if self is root or a descendant of root."""
        rlen = len(root._path)
        if self._path[:rlen] != root._path:
            return False
        return len(self._path) == rlen or self._path[rlen] == sep or root._path == sep

    def is_descendant_of(self, root: "Path") -> bool:
        """Return True if self is a descendant of root, but not root itself."""
        rlen = len(root._path)
        if len(self._path) <= rlen or self._path[:rlen] != root._path:
            return False
        return self._path[rlen] == sep or root._path == sep
...
def ensure_rename(dst: Path, src: Path) -> None:
    if dst._path == src._path:
        ...
        assert parent is not None, (
            "parent is None, so dst is root. Cannot rename to root!"
        )
        ensure_dir(parent)
        ensure_absent(dst)
        src.rename(dst)
...
def ensure_symlink_unsafe(path: Path, orig: str) -> None:
    parent = path.parent()
    assert parent is not None, (
        "path has no parent, so path is root. Cannot symlink root!"
    )
...
@overload
def ensure_file(
    path: Path, mode: Literal["r", "w", "a", "rt", "wt", "at"]
) -> IO[str]: ...
...
def ensure_file(path: Path, mode: str):
    ...
    parent = path.parent()
    assert parent is not None, (
        "path has no parent, so path is root. Cannot create root as a file!"
    )
    assert parent != path, "Path and parent were the same!"
    ensure_dir(parent)
    fd = path.open(mode)
...
def walk(
    *roots: Path, skip: Optional[SkipFunction] = None
