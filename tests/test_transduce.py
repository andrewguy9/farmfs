...
def test_reduceWithAsAggregator():
    assert reduceWith(filter(lambda x: x > 5)(squares(arrayOf)), [], one2ten) == [
        36,
        49,
        64,
        81,
...
def test_reduceWithAsFilter():
    ...
    assert reduceWith(squares(arrayOf), [], one2ten) == [
        1,
        4,
        9,
        16,
...
def test_reduceWithComposition():
    assert reduceWith(squaresOfTheOddNumbers(arrayOf), [], one2ten) == [
        1,
        9,
        25,
        49,
